...
import com.chitts.exceptions.converter.ConverterException;
import com.chitts.exceptions.converter.childs.TypeConverterException;
...
public class ConverterAggregator<T extends Model> {
    ...
    private Map<String, GenericConverter<T, String>> getGenericConvertors() {
        final Map<String, GenericConverter<T, String>> genericConvertors = new HashMap<>();
    ...
    private GenericConverter<T, String> getTypeConverter(final String typeConvertor) throws ConverterException {
        final GenericConverter<T, String> converter = getGenericConvertors().get(typeConvertor.toLowerCase(Locale.ROOT));
        try {
            if (converter == null) {
                throw new TypeConverterException();
            }
        } catch (final TypeConverterException e) {
           throw  new ConverterException(typeConvertor.toUpperCase() + " type is not supported for conversion.", e);
        }
        return converter;
    }

    public String convertTo(final String typeConvertor, final T obj, final Class<T> cl) throws JAXBException, AppException {
        final GenericConverter<T, String> converter = getTypeConverter(typeConvertor);
        return converter.convert(obj, cl);
    }

    public String convertTo(final String typeConvertor, final List<T> list, final Class<T> cl) throws JAXBException, AppException {
        final GenericConverter<T, String> converter = getTypeConverter(typeConvertor);
